...
class Snowflake
{
    ...
    public function setWorkId($workId)
    {
        if ($workId > self::$maxWorkerId || $workId < 0) {
            throw new \think\Exception("worker Id can't be greater than ".self::$maxWorkerId." or less than 0");
        }
    ...
    public function nextId()
    {
        ...
        if ($timestamp < self::$lastTimestamp) {
            throw new \think\Exception("Clock moved backwards.  Refusing to generate id for " . (self::$lastTimestamp - $timestamp) . " milliseconds");
        }
